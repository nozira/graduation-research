...
row = []
#初期化データ読込
with open('HK_conect.csv','r',encoding='utf8') as input:
    for k in range(194):
        d = input.readline()
        brr.append(d.split(','))
    #print(brr[0][0])
#地域ＩＤ読込
with open('HK_ID.txt','r',encoding='utf8') as input_id:
    for k in range(194):
        d = input_id.readline()
        e = d.rstrip()
        row.append(e)
#隣接情報読込　and　配列に格納　after　処理
with open('HK_conect00.txt','r',encoding='utf8') as input_conect:
    c = input_conect.read()
    arr.append(c.split('\n\n'))
    for i in range(188):
